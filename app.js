...
function selectUser(email) {
  let myquert = `SELECT * FROM EP.user where Email = '${email}';`;
...
app.use("/component/", require("./routs/create_component"));
...
app.post("/register", checkNotAuthanticate, async (req, res) => {
  try {
    //refactering the value from the form
      ...
      .then(async () => {
        if (Error.length > 0) {
          ...
          db.query(query, (err, results, fields) => {
            if (err) {
              logger(`[158] There is a connection err ${err}`);
              ...
              req.flash(
                "success",
                "You now register, contact admin for activation"
              );
              res.redirect("/login");
            }
...
app.post("/addserver", (req, res) => {
  //refactering the value from the form
  const { hostname, ip, recstatus, recordingmode } = req.body;
  const {
    sqlhostname,
    sqlip,
    sqlinstant,
